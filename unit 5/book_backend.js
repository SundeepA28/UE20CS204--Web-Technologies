...
app.get("/", (req, res) => {
    res.sendFile(path.join(__dirname, "book_collection.html"));
});
...
app.post("/login/update", (req, res) => {
    ...
    const main2 = async () => {
        MongoClient.connect(uri, async (err, client) => {
            if (err) throw err;
            const check = await 
            client.db("book_collection").collection("details").findOne(query2);
            if (check == null) {
                res.send(`<h1>Data is not present in the database</h1>`);
            } 
            else {
                await 
                client.db("book_collection").collection("details").updateOne(query2, {$set: update,});
                client.close();
                res.send(`
...
app.get("/update", (req, res) => {
    res.sendFile(path.join(__dirname, "Update_books.html"));
});
